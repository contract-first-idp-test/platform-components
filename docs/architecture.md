...
configured with the installer-selected `DEMO_USER_USERNAME` and `DEMO_USER_PASSWORD` and belongs to
...
`bootstrap/secrets.env`; other platform-owned passwords and client secrets are generated by ESO and
aggregated in `platform-generated-secrets` with `CreatedOnce`, orphan, and retain semantics.
