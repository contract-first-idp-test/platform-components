...
3. Configure GitHub and credentials.
   ...
   Create the ignored credential file and populate the externally issued GitHub App values, the
   documented non-secret usernames/emails, and a strong demo-user password of your choice in
   `DEMO_USER_PASSWORD`. Other internal passwords and client secrets are generated by External
   Secrets Operator; do not manufacture those with `openssl rand`.
   ...
   The value after `DEMO_USER_PASSWORD=` is read literally by `oc --from-env-file`; keep it on one
   line and do not wrap it in shell quotes.
...
through Keycloak with the `DEMO_USER_USERNAME` and `DEMO_USER_PASSWORD` chosen in the ignored
`bootstrap/secrets.env` file. The password no longer needs to be extracted from a generated Secret.

Other service credentials such as `QUAY_ADMIN_PASSWORD` and `GITEA_ADMIN_PASSWORD` remain generated
in `platform-generated-secrets` and can be extracted when their services are enabled:

```bash
oc extract secret/platform-generated-secrets -n cf-idp-secrets \
  --keys=QUAY_ADMIN_PASSWORD --to=-
```

Restrict terminal history and screen sharing while handling credentials.
